...
const discord = {
  ...
  botName: 'Milo',
  servers: [
    { name: 'Milo HQ', id: '1477080375812423700', members: 2, channels: 5 }
  ],
  status: 'online'
};
...
export async function GET(request: Request) {
  ...
  if (type === 'crons') return NextResponse.json({ cronJobs });
  if (type === 'projects') return NextResponse.json({ projects });
  if (type === 'discord') return NextResponse.json({ discord });

  return NextResponse.json({ agents, cronJobs, projects, discord, system: { gateway: true, tailscale: true } });
}
